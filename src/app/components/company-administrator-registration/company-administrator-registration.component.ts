...
export class RegisterCompanyAdministratorComponent {
  ...
  registerCompanyAdministrator(): void {
    ...
    const companyAdministrator: CompanyAdministrator = {
        ...
        firstName: this.registrationCompanyAdministratorForm.value.firstName || "",
    ...
    if (this.registrationCompanyAdministratorForm.valid) {
      ...
      this.companyService.registerCompanyAdministrator(this.companyId, companyAdministrator).subscribe({
        next: () => {
        ...
        error: (error) => {
          if (error.status === 400) {
            ...
            this.registrationCompanyAdministratorForm.get('email')?.setErrors({ 'emailExists': true });
          } else {
            // Handle other errors
